  ...
  <UContainer v-if="post">
    <UPageHeader :title="post.title" :description="post.description">
      <template #headline>
        ...
        <time class="text-gray-500 dark:text-gray-400">{{ new Date(post.date).toLocaleDateString('cs', { year: 'numeric', month: 'short', day: 'numeric' }) }}</time>
      </template>
    ...
    <UPage>
      ...
      <template #right>
        <UContentToc v-if="post.body && post.body.toc" :links="post.body.toc.links" title="Osnova" />
      </template>
    </UPage>
  </UContainer>
</template>
